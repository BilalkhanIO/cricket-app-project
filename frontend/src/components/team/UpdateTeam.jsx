...
const UpdateTeam = ({ teamId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append('players', JSON.stringify(formData.players));
    ...
    try {
      ...
      alert('An error occurred while updating team');
    }
  };

  return (
    <div className="container mx-auto py-8">
      <h1 className="text-3xl font-bold mb-4">Update Team</h1>
      <form onSubmit={handleSubmit}>
        {/* Add form fields for team details */}
        <input type="file" name="logo" onChange={handleFileChange} />
        <button type="submit" className="bg-blue-500 text-white px-4 py-2 rounded mt-4">Update Team</button>
      </form>
    </div>
  );
};
