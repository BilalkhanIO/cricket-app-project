...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!event) {
      return NextResponse.json({ error: "Ball event not found" }, { status: 404 });
    }
    ...
    if (session.user.role === ROLE.SCORER && event.innings.match.scorerId !== session.user.id) {
    ...
    const replayedInnings = await replayInningsState(event.innings.id);
    ...
    return NextResponse.json({
      ball: updatedBall,
      innings: replayedInnings,
      message: "Ball event updated and innings replayed",
    });
  } catch (error) {
    console.error("Scoring edit error:", error);
    return NextResponse.json({ error: "Failed to edit ball event" }, { status: 500 });
  }
}
